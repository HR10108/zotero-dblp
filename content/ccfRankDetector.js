...
Zotero.CCFRankDetector = {
    ...
    loadCCFData: async function() {
        try {
            ...
            Zotero.debug("CCF data loaded successfully");
        } catch (error) {
            Zotero.debug("Error loading CCF data: " + error);
    ...
    detectRank: function(item) {
        ...
        if (!publicationName) {
            Zotero.debug("No publication venue found for item: " + item.getField("title"));
            return null;
        }
    ...
    // Add CCF rank information to the item
    addRankToItem: function(item, rankInfo) {
        if (!rankInfo) return item;
        
        try {
            // Set CCF rank as a tag
            const tagString = "CCF-" + rankInfo.rank;
            item.addTag(tagString);
            
            // Store CCF data in extra field for potential use in displaying or exporting
            ...
            Zotero.debug("Error adding CCF rank to item: " + error);
        }
